...
// Define the initial state based on the T6_4ResourcePerson model
const initialState = {
  faculty_name: "",
  invited_by: "",
  lecture_title: "",
...
export default function T6_4Form({ session, year, editData, onSuccess }) {
  const {
    ...
    endpoint: formConfig["T6.4"].endpoint,
    initialState,
    editData,
    onSuccess,
    session,
    ...
    >
      <Paper variant="outlined" sx={{ p: 4, borderRadius: 2 }}>
        <Typography variant="h6" gutterBottom>
          {isEditMode ? "Edit Resource Person Invite (T6.4)" : "Add Resource Person Invite (T6.4)"}
        </Typography>
        ...
        >
          ...
          <TextField name="invited_by" label="Invited By" value={formData.invited_by} onChange={handleChange} size="small" />
          <TextField name="lecture_title" label="Title/Subject of Lecture Delivered" value={formData.lecture_title} onChange={handleChange} size="small" sx={{ gridColumn: '1 / -1' }} />
          ...
          <TextField
            ...
            size="small"
            inputProps={{ step: "0.5", min: "0" }} // Allows decimal input like 1.5
          />
