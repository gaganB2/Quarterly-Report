...
const formSchema = z.object({
  ...
  psv_name: z.string().min(1, "PSU name is required"),
  package_offered: z.coerce.number().min(0, "Package must be a positive number"),
  joining_year: z.coerce.number().min(1900, "Year must be after 1900").max(new Date().getFullYear() + 5, 'Year seems too far in the future'),
...
const initialState = {
  student_name: "", batch: "", exam_name: "", registration_number: "", psv_name: "",
  package_offered: 0, joining_year: new Date().getFullYear(), offer_ref_number: "",
  contact_details: "", email: "", mobile: "", social_profile_link: "", proof_link: "",
  quarter: "", year: "",
};

export default function S4_3Form({ session, year, editData, onSuccess }) {
  ...
  const onFormSubmit = async (data) => {
    try {
      const endpoint = formConfig["S4.3"].endpoint;
      const url = isEditMode ? `${endpoint}${editData.id}/` : endpoint;
      const method = isEditMode ? "put" : "post";
      await apiClient[method](url, data);
      enqueueSnackbar(`PSU selection entry ${isEditMode ? "updated" : "added"} successfully!`, { variant: "success" });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Paper variant="outlined" sx={{ p: 3, borderRadius: 2 }}>
        <Typography variant="h6" gutterBottom>{isEditMode ? "Edit Govt/PSU Selection (S4.3)" : "Add Govt/PSU Selection (S4.3)"}</Typography>
        <Box component="form" onSubmit={handleSubmit(onFormSubmit)} sx={{ mt: 3 }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}><TextField {...register("psv_name")} label="Name of the PSU" size="small" fullWidth error={!!errors.psv_name} helperText={errors.psv_name?.message} /></Grid>
            ...
            <Grid item xs={12} sm={6}><TextField {...register("joining_year")} label="Year of Joining" type="number" size="small" fullWidth error={!!errors.joining_year} helperText={errors.joining_year?.message} inputProps={{ min: 1900 }} /></Grid>
            <Grid item xs={12}><TextField {...register("offer_ref_number")} label="Reference Number of Joining Letter" size="small" fullWidth error={!!errors.offer_ref_number} helperText={errors.offer_ref_number?.message} /></Grid>
