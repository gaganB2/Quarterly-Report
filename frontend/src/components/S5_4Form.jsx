...
const formSchema = z.object({
  ...
  establishment_year: z.coerce.number().min(1900, "Year must be after 1900").max(new Date().getFullYear(), 'Year cannot be in the future'),
  organization_details: z.string().min(1, "Organization details are required"),
...
export default function S5_4Form({ session, year, editData, onSuccess }) {
  ...
  const onFormSubmit = async (data) => {
    try {
      const endpoint = formConfig["S5.4"].endpoint;
      const url = isEditMode ? `${endpoint}${editData.id}/` : endpoint;
      const method = isEditMode ? "put" : "post";
      await apiClient[method](url, data);
      enqueueSnackbar(`Entrepreneurship entry ${isEditMode ? "updated" : "added"} successfully!`, { variant: "success" });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Paper variant="outlined" sx={{ p: 3, borderRadius: 2 }}>
        <Typography variant="h6" gutterBottom>{isEditMode ? "Edit Student Entrepreneurship (S5.4)" : "Add Student Entrepreneurship (S5.4)"}</Typography>
        <Box component="form" onSubmit={handleSubmit(onFormSubmit)} sx={{ mt: 3 }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}><TextField {...register("establishment_year")} label="Year of Establishment" type="number" size="small" fullWidth error={!!errors.establishment_year} helperText={errors.establishment_year?.message} inputProps={{ min: 1900 }} /></Grid>
            <Grid item xs={12}><TextField {...register("organization_details")} label="Name of Organization with Address & Website" size="small" multiline rows={3} fullWidth error={!!errors.organization_details} helperText={errors.organization_details?.message} /></Grid>
            <Grid item xs={12}><TextField {...register("sector")} label="Sector" size="small" fullWidth error={!!errors.sector} helperText={errors.sector?.message} /></Grid>
